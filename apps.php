...
if($_SERVER['REQUEST_METHOD']==='POST'){
  ...
  if(isset($_POST['create_app'])){
    $pdo->prepare("INSERT OR IGNORE INTO applications (name,secret_key,description,expires_at) VALUES (?,?,?,?)")->execute([trim($_POST['app_name']),generateSecret(),trim($_POST['app_desc']??''),$_POST['expires_at']?:null]);
    logActivity($pdo,'app_create',"App '{$_POST['app_name']}' created");
    sendDiscordEmbed('app_create','📦 App Created',"Name: `{$_POST['app_name']}`\nSecret: `".generateSecret()."`");
  }
  if(isset($_POST['bulk_status'])){
    $ids=$_POST['app_ids']??[]; $st=$_POST['new_status'];
    if($ids) $pdo->prepare("UPDATE applications SET status=? WHERE id IN (".implode(',',array_fill(0,count($ids),'?')).")")->execute([$st,...$ids]);
  }
...
<form method="POST" style="display:flex;gap:10px;margin:10px 0"><select name="new_status"><option value="active">Active</option><option value="maintenance">Maintenance</option><option value="disabled">Disabled</option></select><button type="submit" name="bulk_status">🔄 Bulk Status</button><button type="submit" name="export_all">📥 Export All Keys</button></form>
