<?php require 'core.php'; $pdo=getPDO();
...
if(!isLoggedIn()): ?>
<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><title>CordAuth | Login</title>
...
</style></head><body><form method="POST"><h2 style="margin:0 0 15px;text-align:center">🔐 CordAuth</h2><?php if(!empty($error)) echo "<p class='err'>$error</p>"; ?><input type="password" name="login_pass" placeholder="Password" required><button type="submit">Login</button><p style="font-size:.8rem;color:var(--muted);text-align:center;margin-top:10px">Default: <code>password</code></p></form></body></html>
<?php exit; endif;
$pdo=getPDO(); $now=time(); $stats=[
  ...
  'keys'=>['total'=>getStat($pdo,"SELECT COUNT(*) FROM keys"),'active'=>getStat($pdo,"SELECT COUNT(*) FROM keys WHERE status='active' AND (expires_at IS NULL OR expires_at>datetime('now'))"),'used'=>getStat($pdo,"SELECT COUNT(*) FROM keys WHERE usage_count>0")]
];
...
?>
<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><title>CordAuth | Dashboard</title>
...
<div class="container">
<h1>📊 <?=$pdo->query("SELECT value FROM settings WHERE key='portal_title'")->fetchColumn()?> Overview</h1>
<div class="grid">
<div class="card"><h3>📦 Applications</h3><p><?=$stats['apps']['total']?> <span style="color:#22c55e;font-size:.8rem">✓ <?=$stats['apps']['active']?></span></p></div>
<div class="card"><h3>🔑 Total Keys</h3><p><?=$stats['keys']['total']?></p></div>
<div class="card"><h3>🟢 Active</h3><p style="color:#22c55e"><?=$stats['keys']['active']?></p></div>
<div class="card"><h3>📈 Used</h3><p style="color:#f59e0b"><?=$stats['keys']['used']?></p></div>
</div>
<div class="charts">
...
const st=<?= $chartData ?>;
new Chart(ctxP,{type:'pie',{labels:Object.keys(st.status),datasets:[{data:Object.values(st.status),backgroundColor:['#22c55e','#f59e0b','#ef4444','#64748b']}]},{plugins:{legend:{labels:{color:'#94a3b8'}}}});
new Chart(ctxD,{type:'doughnut',{labels:Object.keys(st.hwid),datasets:[{data:Object.values(st.hwid),backgroundColor:['#3b82f6','#94a3b8']}]},{plugins:{legend:{labels:{color:'#94a3b8'}}}});
