...
if($_SERVER['REQUEST_METHOD']==='POST'){
  ...
  if($a==='generate'){
    ...
    logActivity($pdo,'key_gen',"$cnt keys generated for app {$_POST['app_id']}");
  } elseif($a==='import'){
...
<div class="form-box"><h3>📥 Import Keys (One per line)</h3><form method="POST"><select name="app_id" required><?php foreach($apps as $a) echo "<option value='{$a['id']}'>{$a['name']}</option>"; ?></select><textarea name="import_text" rows="3" placeholder="KEY-XXXX\nKEY-YYYY..."></textarea><button type="submit" name="action" value="import">Upload Keys</button></form></div>
